...
const GYRO_NUM_SAMPLES: i32 = 5000;
const ACCEL_NUM_SAMPLES: i32 = 2000;
const MAG_NUM_SAMPLES: i32 = 2000;
const ONE_G_FOR_2_G_FULL_SCALE : f32 = 16385.0;
...
pub fn calibrate_gyroscope(mpu9250 : &mut mpu9250::MPU9250) -> Result<mpu9250::GyroscopeMeasurement> {
    ...
    let mut gyro_sum_x : i32 = 0;
    let mut gyro_sum_y : i32 = 0;
    let mut gyro_sum_z : i32 = 0;
    for _ in 0..GYRO_NUM_SAMPLES {
        let gyro_data = mpu9250.get_rotation()?;
        gyro_sum_x += gyro_data.x as i32;
        gyro_sum_y += gyro_data.y as i32;
        gyro_sum_z += gyro_data.z as i32;
    }
    let gyro_offset_x = gyro_sum_x as f32 / GYRO_NUM_SAMPLES as f32;
    let gyro_offset_y = gyro_sum_y as f32 / GYRO_NUM_SAMPLES as f32;
    let gyro_offset_z = gyro_sum_z as f32 / GYRO_NUM_SAMPLES as f32;
...
pub fn calibrate_accelerometer(mpu9250 : &mut mpu9250::MPU9250) -> Result<mpu9250::AccelerometerMeasurement> {
    ...
    let mut accel_sum_x : i32 = 0;
    let mut accel_sum_y : i32 = 0;
    let mut accel_sum_z : i32 = 0;
    for _ in 0..ACCEL_NUM_SAMPLES {
        let accel_data = mpu9250.get_acceleration()?;
        accel_sum_x += accel_data.x as i32;
        accel_sum_y += accel_data.y as i32;
        accel_sum_z += accel_data.z as i32;
    }
    let accel_offset_x = accel_sum_x as f32 / ACCEL_NUM_SAMPLES as f32;
    let accel_offset_y = accel_sum_y as f32 / ACCEL_NUM_SAMPLES as f32;
    let mut accel_offset_z = accel_sum_z as f32 / ACCEL_NUM_SAMPLES as f32;
    // Handle either z axis up or z axis down
    if accel_offset_z > 0.0 {
