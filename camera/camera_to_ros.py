...
if __name__ == '__main__':
    try:
        ...
        ros_topic = "camera/compressed_image"
        image_format = "jpeg"  # Consistent with the capture format
        frame_id = "camera_optical_frame"

        # Zenoh setup
        conf = zenoh.config.Config()
        conf.connect.endpoints.append("tcp/0.0.0.0:7447")  # Adjust if your ROS 2 node is on a different machine/port
        z = zenoh.open(conf)
        publisher = z.publisher(ros_topic)
